...
std::string hipHeader() {
    return
        ...
        "\n"
        "#define SYNC CHECK(hipStreamSynchronize(stream))\n"
        "#define CHECK(ans) { gpuAssert((ans), __FILE__, __LINE__); }\n"
        ...
        "#endif\n"
        "\n"
        "#define K(num) hipLaunchKernelGGL(k##num, dim3(1 + (numInstances[num] - 1) / numThreads), dim3(numThreads), 0, stream, x_d, c_d + constantsOffset[ num ], x_d + outputOffset[ num ], p_d + indexOffset[ num ] )\n"
        "\n"
        "extern \"C\" {\n"
        "EXPORT void init(const unsigned int* indexData, const unsigned int* outIndexData, const RealT* constData);\n"
        "EXPORT void run();\n"
...
const std::string hipFooter =
"__global__ void gatherResult(RealT* x, RealT* d, const unsigned int* outIdsK, const int n) {\n"
"    const unsigned int i = hipThreadIdx_x + hipBlockDim_x * hipBlockIdx_x;\n"
"    if(i < n) d[i] = x[outIdsK[i]];\n"
"}\n"
"\n"
"EXPORT void init(const unsigned int* indexData, const unsigned int* outIndexData, const RealT* constData) {\n"
...
"    CHECK(hipMalloc(&x_d, sizeof(RealT) * numTotalMemory));\n"
"\n"
"    const size_t maxOutLen = numOuts ? *max_element(outputIndexLength, outputIndexLength + numOuts) : 0;\n"
"    CHECK(hipMalloc(&gatherBuffer_d, sizeof(RealT) * maxOutLen));\n"
"\n"
"    CHECK(hipMalloc(&p_d, sizeof(INDEX) * numIndizes));\n"
"\n"
"    if(indexData) {\n"
"        CHECK(hipMemcpy(p_d, indexData, sizeof(INDEX) * numIndizes, hipMemcpyHostToDevice));\n"
"    } else {\n"
"        unsigned int* p  = new unsigned int[numIndizes];\n"
"        std::ifstream file(\"indexData\", ios::in | ios::binary);\n"
"        file.read((char*)p, sizeof(unsigned int) * numIndizes);\n"
"        file.close();\n"
"        CHECK(hipMemcpy(p_d, p, sizeof(INDEX) * numIndizes, hipMemcpyHostToDevice));\n"
"        delete[] p;\n"
"    }\n"

"\n"
"    CHECK(hipMalloc(&c_d, sizeof(RealT) * numConst));\n"
"\n"
"    if(constData) {\n"
"        CHECK(hipMemcpy(c_d, constData, sizeof(RealT) * numConst, hipMemcpyHostToDevice));\n"
"    } else {\n"
"        RealT* c = new RealT[numConst];\n"
"        std::ifstream file(\"constData\", ios::in | ios::binary);\n"
"        file.read((char*)c, sizeof(RealT) * numConst);\n"
"        file.close();\n"
"        CHECK(hipMemcpy(c_d, c, sizeof(RealT) * numConst, hipMemcpyHostToDevice));\n"
"        delete[] c;\n"
"    }\n"
"\n"
"    CHECK(hipMalloc(&outIds_d, sizeof(INDEX) * numOutputValues));\n"
"\n"
"    if(outIndexData) {\n"
"        CHECK(hipMemcpy(outIds_d, outIndexData, sizeof(INDEX) * numOutputValues, hipMemcpyHostToDevice));\n"
"    } else {\n"
"        INDEX* out = new INDEX[numOutputValues];\n"
"        std::ifstream file(\"outIndexData\", ios::in | ios::binary);\n"
"        file.read((char*)out, sizeof(unsigned int) * numOutputValues);\n"
"        file.close();\n"
"        CHECK(hipMemcpy(outIds_d, out, sizeof(INDEX) * numOutputValues, hipMemcpyHostToDevice));\n"
"        delete[] out;\n"
"    }\n"
"\n"
"}\n"
"\n"
"EXPORT void setArg(const RealT* d, int i) {\n"
"    CHECK(hipMemcpy(x_d + argStart[i], d, sizeof(RealT) * argLength[i], hipMemcpyHostToDevice));\n"
"}\n"
"\n"
"EXPORT void getResult(RealT* d, int k) {\n"
"    const size_t n = outputIndexLength[k];\n"
"    gatherResult<<<1 + (n - 1) / numThreads, numThreads, 0, stream>>>(x_d, gatherBuffer_d, outIds_d + outputIndexStart[k], n);\n"
"    CHECK(hipMemcpy(d, gatherBuffer_d, sizeof(RealT) * n, hipMemcpyDeviceToHost));\n"
